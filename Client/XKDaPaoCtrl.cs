...
//					          +pointScript[i].transform.parent.name);
	...
	List<XKNpcHealthCtrl> YouTongScriptList;
	public void ClearYouTongScriptList()
	{
		if (YouTongScriptList != null) {
			YouTongScriptList.Clear();
		}
	}
	...
	public void CallListYouTongDamage(float timeVal)
	{
		if (YouTongScriptList != null && YouTongScriptList.Count > 0) {
			TimeYouTongDamage = timeVal;
			StartCoroutine(LoopYouTongDamage());
		}
	}

	IEnumerator LoopYouTongDamage()
	{
		int countDamage = 0;
		bool isContinue = true;
		do {
			//Debug.Log("***time "+Time.realtimeSinceStartup+", countDamage "+countDamage);
			yield return new WaitForSeconds(TimeYouTongDamage);
			//Debug.Log("time "+Time.realtimeSinceStartup+", countDamage "+countDamage);
			if (countDamage < YouTongScriptList.Count && YouTongScriptList[countDamage] != null) {
				YouTongScriptList[countDamage].OnDamageNpc(PlayerAmmoType.DaoDanAmmo, PlayerEnum.Null);
			}

			countDamage++;
			if (countDamage >= YouTongScriptList.Count) {
				isContinue = false;
				//Debug.Log("stop youTongDamage...");
				yield break;
			}
		} while(isContinue);
	}
